...
test("source preserves accessible and responsive contracts", async () => {
  ...
  assert.match(projectStage, /import\("gsap"\)/);
  assert.match(projectStage, /gsap\.quickTo/);
  assert.match(projectStage, /\(hover: hover\) and \(pointer: fine\)/);
  assert.match(projectStage, /list\.addEventListener\("pointermove", handlePointerMove\)/);
  assert.match(projectStage, /list\.removeEventListener\("pointermove", handlePointerMove\)/);
  assert.match(projectStage, /data-project-list/);
  assert.match(projectStage, /data-project-row/);
  assert.match(projectStage, /onPointerEnter=\{\(\) => setActiveIndex\(index\)\}/);
  assert.match(projectStage, /onFocusCapture=\{\(\) => setActiveIndex\(index\)\}/);
  assert.match(projectStage, /aria-live="polite"/);
  assert.match(projectStage, /project-showcase-mobile/);
  assert.match(projectStage, /data-project-mobile-card/);
  assert.match(projectStage, /alt=\{`\$\{project\.name\} system artwork`\}/);
  assert.match(css, /\.project-showcase-desktop\s*\{\s*display:\s*none/);
  assert.match(css, /@media \(min-width:\s*900px\)[\s\S]*?\.project-showcase-desktop\s*\{[\s\S]*?display:\s*grid/);
  assert.match(css, /\.project-cursor-preview/);
  assert.match(css, /\.project-mobile-card/);
  ...
  assert.match(browserAudit, /projectStage\.locator\("\[data-project-row\]"\)/);
  ...
  assert.match(siteMotion, /\[data-project-row\]/);
  assert.match(siteMotion, /\[data-project-stage-preview\]/);
  assert.match(siteMotion, /\[data-project-mobile-card\]/);
